...
def index():
    ...
    if request.method == 'POST':
        if request.form.get('user'):
            ...
            limitador=len(self.vecindarioAux)/5

            mycursor = mydb.cursor()
            consulta6="SELECT DISTINCTROW movieID FROM ratings WHERE userID IN (%s) and movieID NOT IN %s GROUP by movieID having COUNT(movieID)>"+str(limitador)
...
def indexitem():
    ...
    if request.method == 'POST':
        if request.form.get('user'):
            ...
            ######################FORMULA ITEM ITEM##########################################################
            mycursor = mydb.cursor()
            mycursor.execute("SELECT movieID FROM ratings WHERE movieID NOT IN (SELECT movieID FROM ratings WHERE userID=%s) limit 10", (userID,))
            ...
            for peliculaNoVista in PeliculasNoVistas:
                ...
                for peliculaVista in PeliculasVistas:
                    ...
                    for ID in usuariosComunes:
                        contador=0
                        mycursor = mydb.cursor()
                        mycursor.execute("SELECT rating FROM ratings WHERE movieID=%s limit 5", (peliculaVista[0],))
                        ...
                        print("SIMILITUD")
                        print(similitud)
                        if float(similitud) >= float(umbral):
                            self.vecindarioAux.append(peliculaNoVista[0])
                            self.PearsonVecindario.append((ID, similitud))
